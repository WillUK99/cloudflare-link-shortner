...
export function useClickSocket() {
  ...
  useEffect(() => {
    const connect = () => {
      ...
      console.log(import.meta.env.VITE_BASE_HOST)
      const socket = new WebSocket(`${protocol}//${import.meta.env.VITE_BASE_HOST}/click-socket`)
      ...
      socket.onmessage = (event) => {
        console.log(event)
        const data = durableObjectGeoClickArraySchema.parse(JSON.parse(event.data))
        addClicks(data)
      }
