...
export async function getRecentClicks(sqlStorage: SqlStorage, offsetTime: number = 0, limit: number = 50) {
  const query = `
...
export async function deleteClicksBefore(sqlStorage: SqlStorage, time: number) {
  const query = `
